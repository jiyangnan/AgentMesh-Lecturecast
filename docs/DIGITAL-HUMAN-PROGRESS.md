...
- §5.5e5d-c maintenance wiring —— **round-3 NOT LOCKABLE（5 blocker）→ round-4 全闭合 → round-4 NOT LOCKABLE（3 gap）→ round-5 全闭合 → round-5 NOT LOCKABLE（2 blocker：`BaseException` __repr__ 逃逸 `except Exception` / broken-topology gap）→ round-6 全闭合（109 测 + diagnostic 不调 `repr()` 改 `len()` + 抽 `_DELETION_WITNESS_SUBQUERY_SQL` 共享常量 + domain (c) `NOT EXISTS(witness)`）→ round-6 NOT LOCKABLE（1 blocker：`_valid_tally` 的 `set(keys)` 重新 hash + diagnostic `type(x).__name__` 两同族 BaseException 逃逸洞）→ round-7 全闭合（`_valid_tally` 加 str-guard 在 hash 前拒非 str 键 + diagnostic non-dict 分支改固定 `"non-dict"` + domain (c) docstring 拆 c1/c2 修正过度声称；111 测，1169 全绿），待 Codex round-7 锁定复审**
...
§5.5e5c（capability wiring）+ §5.5e5d-a（doctor v1.1）+ §5.5e5d-b（canary harness）**已锁定**（6 轮 + a/b 各自 Codex 审阅后锁定）。客户端 **1169 测试全绿**。分支 `feat/digital-human-protocol-v1_1`。

---

## 下次会话接续点（交接）—— e5d-c maintenance 待 Codex round-7

**当前状态**：e5c/e5d-a/e5d-b 全部锁定；e5d-c maintenance wiring —— **round-1→round-6 轨迹见下；round-6 NOT LOCKABLE（1 blocker：`_valid_tally` 的 `set(keys)` 重 hash + diagnostic `type(x).__name__` 两同族 BaseException 逃逸洞）→ round-7 全闭合，1169 测全绿，待发 Codex round-7 锁定复审**。round-7 闭合（见 `docs/e5cd-design.md` §1.13b）：(1) **blocker B1（`__hash__`/`__name__` 同族 totality 洞）**—— round-6 删了 diagnostic 的 `repr()`，但 `_valid_tally`（在 diagnostic 之前跑）仍调 `set(tally.keys())` 重新 hash 每键，且 diagnostic non-dict 分支仍 `type(x).__name__` —— 两洞都能被 `BaseException`（`KeyboardInterrupt`/`SystemExit`）子类的 hostile dunder 逃逸（`except Exception` 兜不住；catch `BaseException` 被禁会吞 Ctrl+C）。**治本修（非 whack-a-mole）**：`_valid_tally` 加 `if any(type(k) is not str for k in tally): return False` 守卫在 hash 任一键**之前** —— 论证链全 C-builtin（迭代 plain dict 的 builtin `__iter__` 不 hash；`type(k) is str` 身份比较不调 dunder；builtin str 的 `str.__hash__` 是 C builtin 不能抛）；diagnostic non-dict 分支 `type(x).__name__` → 固定 `"non-dict"`（不读 metaclass descriptor）。(2) **doc-only nuance（domain c 过度声称）**—— Codex round-6 item-4 指 round-6 把 domain (c) 描述为 "exclusively static corruption / primitive-unreachable, zero on any producer-valid journal" 不准确：`withdraw`（consent.py:591）不清 op lease，leased+无 video 的 op 上被 enqueue 的 `consent_withdrawal` 资产是 producer-valid 瞬态 blocked-pending（非 corruption），count 只在 SETTLED journal 上为 0。docstring + SQL COMMENT 拆 (c1) static corruption + (c2) transient blocked-pending。**元教训**：round-4 repr → round-5 except Exception → round-6 删 repr → round-6 仍漏 hash/name —— 治本不是再堵一个 dunder，是**只调 provably 不能抛的 C-builtin 操作**（type/len/is/builtin dict iter），守卫在触发用户 dunder 之前。两 RED-then-GREEN probe（`_StatefulHashKey` + `_HostileNameValue`）先复现 round-6 漏洞再验证 round-7 闭合。**纯加法只读诊断 + 文档，不改任何 locked mutation primitive**。
