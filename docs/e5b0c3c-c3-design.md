...
> 本稿含：设计 + 盲预测 + 26 bypass risks 映射（12 调研 + R13 round-1 + R14 round-2 + R15 round-3 + R16 round-4 + R17 round-5 + R18 round-6[6 子类] + R19/R20 round-7[2 子类] + R21 round-8[1 子类] + R22 round-9[1 子类] + R23 round-10[1 子类] + R24 round-11[1 子类] + R25 round-12[1 子类] + R26 round-13[3 子类]）+ 68 测试矩阵 + Codex 问题
...
## 4. bypass risks × 防御映射（12 来自调研 + R13 round-1 + R14 round-2 + R15 round-3 + R16 round-4 + R17 round-5 + R18 round-6 + R19/R20 round-7 + R21 round-8 + R22 round-9 + R23 round-10 + R24 round-11 + R25 round-12 + R26 round-13）
...
| **R26** | **claim↔apply 对偶接缝（round-13）：ASSET claim/apply 漏 op-level cross-tx 复查——只复查 resource 自身（矩阵/role/own lease），从不读 op.download_status 或 video topology（c1 层化「asset processor delivery-agnostic」）。claim（tx1）↔apply（tx2）跨 tx 失效：(F3) UPDATE op.download_status='not_started' → post_download asset 在未验证 op 上仍被删；(F4) INSERT 第 2 个 deleted video（COUNT 1→2）→ 双 video corrupt op 上 asset 仍被删；(F5) INSERT live（not_started）video → resolver 释放 tail 后 §3.5 order 被破、asset 仍被删。video apply round-12 已复查这三件套，asset apply 漏镜像——同一「原则陈述正确 ≠ 实现穷举」病在 asset 侧的复现** | **加共享 staticmethod `_asset_post_download_op_level_ok(conn, op_id, download_status)`：F3=download_status=='verified' / F4=COUNT(video)<=1（非 ==1，B2 witness 是 asset，op 可合法 0 video）/ F5=无 non-deleted 非 reusable video；单 SELECT 两标量子查询、NULL fail-closed。在 asset claim（`not_ready`）与 asset apply（`fence_conflict`，lease 不清）两处门禁，仅 post_download（consent_withdrawal 短路豁免，交付无关）；`force` 线程化穿透 claim/apply/processor/coordinator（force 模式豁免 F3/F4/F5，§3.5 privacy-emergency）。打破 c1 layering：post_download asset claim/apply 变 delivery-aware。**Codex round-13 修正 F5 域**：F5 用 `created_by_operation_id` 域（镜像 resolver @2399-2406，不需 ref），F4 留 refs 域（authority 是 _single_video/B2 count 不变量——改 created_by 会 over-block 合法双 deleted-video op）；F4/F5 跨域因 authority 不同** | **T19a/T19b/T19c/T19d + T19-ctrl-consent + T19-ctrl-force** |

## 5. 测试矩阵（68，新文件 `tests/test_deletion_coordinator.py`）
...
- **T19a**【round-13 回归 · ASSET claim/apply 漏复查 op.download_status（F3）】verified op + audio asset（无 video）；**直接驱动 repo**（篡改落在 claim tx 与 apply tx 之间）：tx1 `claim_asset_deletion_in_tx` → claimed → 独立连接 `UPDATE heygen_operations SET download_status='not_started'`（reason/retention/single-video 不变）→ tx2 `apply_asset_deletion_outcome_in_tx(AssetDeleteResult('deleted'), expected_remote_id=claim.remote_id)`。asset apply 漏复查 download_status 时：post_download asset 在未验证 op 上仍被删（resource→deleted）；修后 asset apply（镜像 video apply round-12 + 共享 `_asset_post_download_op_level_ok`）复查 download_status=='verified' → fence_conflict + resource 仍 deletion_pending/post_download + **asset lease 未清**（OWNER 持有）。探针 /tmp/apply-seam-probe-r13-mine.py F3=BYPASS 独立复现。（asset mirror of T18b。）
- **T19b**【round-13 回归 · ASSET claim/apply 漏复查 single-video count（F4）】verified op + 1 个 seed deleted video + audio asset；tx1 claim → claimed（COUNT==1 过）→ 独立连接 INSERT 第 2 个 deleted video（COUNT 1→2，两 deleted 故 F5 仍 clean）→ tx2 apply。asset apply 漏复查 count 时：双 video corrupt op 上 asset 仍被删；修后复查 COUNT(video)<=1（**非 ==1**——asset witness，op 可合法 0 video；不变量是「at most one」）→ fence_conflict + resource 仍 deletion_pending + asset lease 未清。探针 F4=BYPASS 独立复现。（asset mirror of T18 single-video；复用 round-9 的 <=1 教训。）
- **T19c**【round-13 回归 · ASSET claim/apply 漏挡 live-video 出现（F5）】verified op + audio asset（0 video）；tx1 claim → claimed → 独立连接 INSERT 一条 live（not_started）video（COUNT 0→1 故 F4 仍 clean，但 non-deleted video 出现）→ tx2 apply。asset apply 漏挡时：resolver 释放 tail 后新 live video 出现、§3.5 order 被破、asset 仍被删；修后复查「无 non-deleted 非 reusable video」→ fence_conflict + resource 仍 deletion_pending + asset lease 未清。探针 F5=BYPASS 独立复现。（reusable video 排除，镜像 resolver skip。）
- **T19d**【round-13 Codex 回归 · F5 域 mismatch（created_by vs refs）】verified op + audio asset（0 video）；tx1 claim → claimed → 独立连接 INSERT 一条 live（not_started）video **带 `created_by_operation_id=op1` 但无 ref 行**（schema-legal incomplete-topology）→ tx2 apply。F5 初版（JOIN refs）漏挡：resolver 用 created_by 选 video_entry（@2399-2406，不需 ref）会 GATE tail，但 F5 refs-JOIN 看不见这条无 ref 的 video → asset 仍 deleted（BYPASS）；修后 F5 改 created_by 域（镜像 resolver 精确 population）→ fence_conflict + resource 仍 deletion_pending + asset lease 未清。探针 /tmp/apply-seam-probe-r13-f5-mine.py F5-no-ref=BYPASS / F5-with-ref=fence_conflict 独立复现（无 ref 是唯一差分）。（F4 刻意留 refs 域——authority 是 _single_video/B2 count 不变量，非 resolver；改 created_by 会 over-block 合法双 deleted-video op。）
- **T19-ctrl-consent**【round-13 合法 control · consent 豁免】未验证 op + consent_withdrawal audio asset（cleanup_required upload）→ claim→apply **仍 deleted**（consent cleanup 交付无关、unverified op 也合法；round-13 op-level 门 reason=='post_download' 短路豁免 consent）。确保 F3 门不过拦 consent cleanup（与 B1 consent 分支 / T17t-ctrl-c / T17w-ctrl-consent 对称）。
- **T19-ctrl-force**【round-13 合法 control · force 豁免】未验证 op + live（not_started）video + audio asset + `force=True` 线程化穿透 claim/apply → claim→apply **仍 deleted**（§3.5 force-cleanup 是 operator privacy-emergency，绕过 video-first 与交付门；F3/F5 都本会挡）。确保 round-13 op-level 门不过拦 force-cleanup（与现有 force-mode 测试一致）。
...
19. ✅ **claim↔apply 对偶接缝（续）：video apply 漏复查 retention/download_status + 不绑 expected_remote_id**（round-12 P1，**Codex round-12 锁定 VIDEO F1/F2 + 确认 F3/F4/F5（asset seam，round-13 修）**）：承接 item 18 Codex round-11 的 F1/F2。机制已在诚实记录 #12 + R25 详述：round-11 原则陈述正确（apply 必须对每个授权不变量在 tx2 用当前行态再校验）但**实现只覆盖 reason**，漏 retention_mode（F1a）/ download_status（F1b）/ remote_id（F2）。我自跑独立探针 /tmp/apply-seam-probe-r12-mine.py（5 case）裁决：F1a/F1b/F2 全 `deleted` BYPASS 复现、CTRL-legacy/CTRL-consent 绿。已修：video apply 加 `expected_remote_id`（kw-only 必填 + 入口非空 str 守卫，镜像 asset apply @2152-2156）；fence SELECT 读 `download_status`；topology SELECT 读 deletion_reason/retention_mode/remote_id 并 WHERE 绑 `r.remote_id=?`（闭 F2）；post_download 复查 retention='ephemeral' AND download_status='verified' AND single-video（consent 豁免）；DeleteProcessor 传 claim.remote_id（镜像 AssetDeletionProcessor @3646）。回归 T18a/T18b/T18c（三 BYPASS）+ 复用 T17w-ctrl-legit/T17w-ctrl-consent（两 control，验证不回归），RED（未修三 BYPASS `assert fence_conflict` 得 `deleted`）→ GREEN（修后全绿）。全量 983 全绿（980+3）→ commit 25e22a4。**Codex round-12 锁定复审结果**：判 **VIDEO F1a/F1b/F2 正确闭合**——apply 读当前 `op.download_status`（@1893-1898）、topology SELECT 读当前 reason/retention/remote_id（@1907-1921）、post_download 三件套 retention='ephemeral' AND download_status='verified' AND single-video（@1941-1946）、`consent_withdrawal` 绕过该 block（@1941，交付无关策略 + T17w-ctrl-consent）、每个拒都在 outcome dispatch（@1951）前返回保 lease、retry bookkeeping 不被新 remote_id WHERE 破坏（claim 递增 attempts @1854-1858 → apply 读当前值 @1907-1909 → retry 决策 `outcome.retryable AND attempts < max_attempts` @1962-1964）；F2 `expected_remote_id` kw-only 必填 + 非空 str 守卫 + WHERE 绑 `r.remote_id=?`（@1913）+ DeleteProcessor 传 claim.remote_id（@3642-3645）。给出 VIDEO apply-time 全 (status, reason) 状态矩阵逐行映射，确认无 schema-reachable 组合意外穿透。**Codex 同时判「不可锁整个子系统」——F3/F4 仍在 candidate-listing → resolver → ASSET-claim seam**：ASSET claim/apply 从不复查当前 `download_status` 或 video cardinality；外加一条相关 seam（**F5**）——resolver → asset-claim sequencing：frozen plan 释放 tail 后可出现一条新 non-deleted video。Codex 标注一条非 bypass 改进项：VIDEO `expected_remote_id` 入口守卫接受 whitespace-only str（asset 用 regex 守卫 @2188-2190）——不是 F2 bypass（仍只匹配真实 whitespace remote_id），但对称性建议收紧。F3/F4/F5 我自跑独立探针 /tmp/apply-seam-probe-r12-f3f4-mine.py 裁决 F3（dl→not_started）= REAL（asset apply 仍 `deleted`），交 round-13 修（见 item 20）。

> **诚实记录 #13（round-13）**：连续第 8 轮「穷举完成/无下一类」被打脸（round-6→7→8→9→10→11→12→13）。round-12 修了 **video** apply 的 op-level cross-tx 复查（retention/download_status/single-video/remote_id），Codex round-12 锁定 video 侧但同时点名 **asset 侧同一接缝完全没修**——ASSET claim/apply 从不读 `download_status`、从不查 video cardinality、从不挡 tail-released 后新出现的 live video。这跟 round-8/9 的 witness B1↔B2 不对称是**同一个根因的第三层**：post_download asset 和 post_download video 一样依赖 op-level 授权（「交付已完成才能清这个 op 的资源」），但 c1 的层化把 asset processor 标成「delivery-agnostic」——asset claim 只看矩阵可推进、不读 op.download_status。这个层化在**单 op 显式删除**语境是对的（caller 已选删这个 op，witness/resolver 已在更早的 closed tx 里授权过），但在 **claim↔apply 跨 tx** 语境失效：witness/resolver 的授权在一个已 COMMIT 的 tx 里，claim（tx1）与 apply（tx2）之间一条 schema-legal UPDATE/INSERT 就能让授权前提消失/反转，而 asset claim/apply 不复查任何 op-level 字段。**关键归因（元级）**：这是 round-11/12「原则陈述正确 ≠ 实现穷举」病在 asset 侧的复现——video apply 已在 round-12 把 post_download 授权三件套（retention/download_status/single-video）+ remote_id 全复查了，但 asset apply（@2176）只复查了 resource 自身（矩阵/role/own lease），**op-level 一件没复查**；asset claim（@2000）同样。我 round-12 修 video 时只盯着 video，没把同一原则投影到 asset 的对偶 apply。**process 转折**：这一轮我**先跑独立探针裁决再动手**（/tmp/apply-seam-probe-r13-mine.py，6 case：F3 dl→not_started / F4 插第 2 个 deleted video / F5 插 live video / CTRL-legacy 无篡改 / CTRL-consent consent+dl 篡改 / CTRL-0video-claim），三 BYPASS 全 `deleted` 复现 + 三 control 全绿 + 0-video op 仍 claim（证实须用 COUNT<=1 非 ==1），再实现共享 helper `_asset_post_download_op_level_ok`（F3=download_status=='verified' / F4=COUNT(video)<=1 / F5=无 non-deleted 非 reusable video，单 SELECT 两条标量子查询，NULL fail-closed），在 asset claim（not_ready）与 asset apply（fence_conflict，lease 不清）两处门禁，`force` 线程化穿透 claim/apply/processor/coordinator（force 模式豁免，§3.5 operator privacy-emergency 绕过 video-first 与交付门）、consent_withdrawal 豁免（reason=='post_download' 短路，交付无关）。**这一轮打破 c1 layering 的第一刀**：asset processor 不再是 delivery-agnostic——post_download asset 的 claim/apply 必须变成 delivery-aware（c1 测试 `_setup_uploaded_for_delete` 需补 `download_status='verified'` precondition）。回归 T19a（F3）/T19b（F4）/T19c（F5）+ T19-ctrl-consent（consent unverified op 仍删）+ T19-ctrl-force（force + live video 仍删），5 全绿；全量 988 全绿（983+5）。**Codex round-13 复审抓 1 个 F5 域 mismatch（连续第 9 轮「修完即锁」被打脸）**：F5（live-video 复查）JOIN `heygen_resource_operation_refs`，但 resolver（`resolve_deletion_plan_in_tx` @2399-2406）用 `created_by_operation_id` 选 video_entry、**不需 ref 行**。一条 `created_by=op` 但**无 ref 行**的 video 会 GATE resolver（tail 不释放 → asset 不该删）却对 F5 的 refs-JOIN 不可见 → asset 仍被删（BYPASS）。我自跑独立探针 /tmp/apply-seam-probe-r13-f5-mine.py 裁决：F5-no-ref=BYPASS（deleted）/ F5-with-ref=fence_conflict / CTRL-legacy=deleted——证实「无 ref 行」是唯一差分。修：F5 改 `created_by_operation_id` 域（镜像 resolver 的精确 population：created_by + kind=video + non-deleted + non-reusable），**F4 留 refs 域**（F4 的 authority 是 `_single_video`/B2 的 count 不变量，非 resolver；改 created_by 会 over-block 合法双 DELETED-video op——resolver 跳过 deleted 正确释放 tail，created_by COUNT 却见 2 冻住）。F4/F5 刻意跨不同域因 authority 不同（claim count 不变量 vs resolver tail-release）。回归 T19d（F5 no-ref video），全量 989 全绿（983+6）。
20. **【round-13 才发现】ASSET claim/apply 漏 op-level cross-tx 复查 → post_download asset 在未验证/双 video/live-video op 上仍被删（R26）**：承接 item 19 Codex round-12 的 F3/F4/F5。ASSET claim（@2000）/apply（@2176）历史只复查 resource 自身（矩阵/role/own lease），从不读 op.download_status 或 video topology——c1 层化「asset processor delivery-agnostic」。这在 witness/resolver 已授权的 closed tx 语境够，但 claim（tx1）↔apply（tx2）跨 tx 失效：(F3) tx1↔tx2 之间 UPDATE `op.download_status='not_started'` → asset 仍 deleted（post_download asset 在未验证 op 上被删）；(F4) INSERT 第 2 个 deleted video（COUNT 1→2，双 video corrupt op）→ asset 仍 deleted；(F5) INSERT 一条 live（not_started）video（resolver 释放 tail 后 §3.5 order 被破）→ asset 仍 deleted。→ 加共享 staticmethod `_asset_post_download_op_level_ok(conn, op_id, download_status)`（F3=verified / F4=COUNT(video)<=1 / F5=无 non-deleted 非 reusable video；单 SELECT 两标量子查询；NULL fail-closed），在 asset claim（`not_ready`）与 asset apply（`fence_conflict`，lease 不清）两处门禁，仅 post_download（consent 短路豁免）；`force` 线程化穿透 claim/apply/processor/coordinator（force 模式豁免 F3/F4/F5，§3.5 privacy-emergency）。回归 T19a/T19b/T19c + T19-ctrl-consent + T19-ctrl-force。**Codex round-13 复审修正 F5 域**：F5 初版 JOIN refs，但 resolver 选 video_entry 用 `created_by_operation_id`（@2399-2406，不需 ref）→ 一条 created_by=op 无 ref 的 video GATE resolver 却对 F5 不可见。修：F5 改 created_by 域（镜像 resolver 精确 population），F4 留 refs 域（authority 是 _single_video/B2 count 不变量，改 created_by 会 over-block 合法双 deleted-video op）。探针 /tmp/apply-seam-probe-r13-f5-mine.py 裁决（F5-no-ref BYPASS / F5-with-ref fence_conflict / CTRL-legacy deleted）证实无 ref 是唯一差分。回归 T19d，全量 989 全绿（983+6）→ commit 6767d1f。**Codex round-13 最终复审：lockable**——F5 fix 正确完整（谓词与 resolver video_entry population 在 schema 非空列下等价，4 case 覆盖；F4 正确留 refs 域，双 deleted 案例走通）；11 行 mutation matrix 全通；全 claim 字段枚举逐一映射 tx2 复查、未复查字段正确归类；video/asset 对称（基数 ==1 vs <=1 已解释）。**c3 锁定**。
