...
LectureCast Community supports two native desktop environments:
...
Optional Director requests contain structured creative inputs only.

## Runtime requirements

Both platforms require Python 3.11+, Node 20+ with npm, and an ffmpeg build with
libass. `lecturecast doctor --project-root <project>` is the source of truth for
renderer readiness.
