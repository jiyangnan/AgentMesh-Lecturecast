...
> 状态：**待环境就绪**。生产 server v0.1.3 未含 M2/M3 路由（openapi 仅 2 条 generation 路由，POST 404）；本地分支 feat/digital-human-edition 有全部 M2/M3 代码但未部署。环境就绪（本地 core+server 全栈 或 生产部署）后按下述顺序执行。
...
### 0c. 前置验证（两种环境通用）
- [ ] `curl <server>/openapi.json` 含 `/v1/director/generations/{gid}/presenter-plan` 与 `/orchestration-plan`
- [ ] client keyring 含 server 签名公钥（`scripts/update_signing_keyring.py --check` 通过）
- [ ] `LECTURECAST_API_KEY` 可解析（env 或 keyring），余额充足（≥30 credits）
...
**断言点**：
- [ ] billing 投影只有 `manifest` milestone，status=charged，cost=10，deducted_credits=10
- [ ] `.lecturecast/production-manifest.json` 验签成功（digest 绑定 creative_brief_digest + capability_digest）
- [ ] `.lecturecast/orchestration-plan.json` **不存在**；`.lecturecast/presenter-plan.json` **不存在**
- [ ] workflow.next_action 为本地渲染（如 `render_required` / M1 后续），**不是** `presenter_plan.create` 或 `orchestration_plan.create`
- [ ] `director generation-presenter-plan` 应被 client 侧门禁拒绝（code=m3_not_ready 或等效），**不得**产生网络调用
...
**断言点**：
- [ ] M3 命令**没有** approval 参数（CLI `--help` 无 `--yes`）
- [ ] `.lecturecast/orchestration-plan.json` 存在；验签通过；`presenter_plan_digest` 字段为 **None**（无 M2，合法）
- [ ] billing 投影：manifest charged(10) + orchestration charged(10)，总 20
- [ ] orchestration-plan 的 `production_manifest_digest` 与落盘 Manifest digest 一致
- [ ] **不出现** presenter-plan 相关文件 / next_action
...
**断言点**：
- [ ] 若卡片能构造 bgm≠none：行为同路径 B（M1+M3，无 M2）
- [ ] 若卡片不可达：记录 N/A，注明 spec †，**不算失败**
...
**断言点**：
- [ ] `director generation-presenter-plan` **不带** `--yes` 应被拒绝（approval 门禁）
- [ ] `.lecturecast/presenter-plan.json` + `.lecturecast/orchestration-plan.json` 均存在且验签通过
- [ ] orchestration-plan 的 `presenter_plan_digest` **非 None**，且 == presenter-plan 落盘 digest
- [ ] billing 投影：manifest(10) + presenter_plan(10) + orchestration(10) = 30
- [ ] M3 命令无 approval 参数（D6 的确认不重复）
...
- [ ] `manifest verify`（client）对落盘 Manifest 验签成功
- [ ] presenter-plan / orchestration-plan 验签成功（created_at 时间窗 + content_expires_at 有效）
- [ ] orchestration-plan.`production_manifest_digest` == Manifest digest；capability_digest 链一致
- [ ] 篡改任一 plan 的 payload → 验签失败（fail-closed）

### 5.2 幂等（每路径抽查）
- [ ] 重跑 M2/M3 命令 → 落盘已存在、不二次扣费（CLI 层幂等，先于 manifest_ready gate）

### 5.3 recovery 抑制（M2/M3 上下文）
- [ ] 构造 insufficient_credits（余额 < 下一步 cost）→ client 不误给 M1 话术；落到 `_resume_error_workflow` 通用 `credit_top_up_required`
- [ ] M3 上下文（orchestration-plan.json 已落盘）insufficient_credits → 同样抑制，不显示 M1 升级话术

### 5.4 全量回归
- [ ] client：`.venv/bin/python -m pytest tests/ -q` 全绿（当前基线 1308 passed）
- [ ] server：`.venv/bin/python -m pytest tests/ -q` 全绿（当前基线 769 passed）
- [ ] ruff：三仓 `ruff check .` 零错误
...
```markdown
## UAT <date> — <环境：local-all-stack / production>

- 环境就绪：openapi 含 M2/M3 路由 ✓ / ✗；keyring 匹配 ✓ / ✗；API key 余额 __ credits
- 路径 A（none+stock+none）：PASS / FAIL — <差异描述>
- 路径 B（none+own_voice）：PASS / FAIL — <差异描述>
- 路径 C（none+stock+bgm≠none）：PASS / N/A(卡片不可达) / FAIL — <差异描述>
- 路径 D（photo）：PASS / FAIL — <差异描述>
- 验签+digest 链：PASS / FAIL
- 幂等：PASS / FAIL
- recovery 抑制：PASS / FAIL
- 全量回归：client __ passed / server __ passed / ruff __
- 问题清单：
  1. [P1/P2/P3] <描述> — <修复建议>
- 结论：READY / NOT-READY（附阻塞项）
```
